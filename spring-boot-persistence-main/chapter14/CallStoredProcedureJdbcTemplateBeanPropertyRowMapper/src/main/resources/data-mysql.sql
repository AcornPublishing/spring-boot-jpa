...
CREATE PROCEDURE FETCH_NICKNAME_AND_AGE_BY_ID(
  IN p_id INT(10), OUT o_nickname CHAR(20), OUT o_age INT(3))
BEGIN
SELECT nickname, age INTO o_nickname, o_age FROM author WHERE id = p_id;
END ^;

CREATE PROCEDURE FETCH_NICKNAME_AND_AGE_BY_GENRE(IN p_genre CHAR(20))
BEGIN
SELECT nickname, age FROM author WHERE genre = p_genre;
END ^;

CREATE PROCEDURE FETCH_NICKNAME_GENRE_AND_AGE_BY_GENRE(IN p_genre CHAR(20))
BEGIN
SELECT nickname, age FROM author WHERE genre = p_genre;
SELECT genre, age FROM author WHERE genre <> p_genre;
END ^;

CREATE PROCEDURE FETCH_AUTHOR_BY_GENRE(IN p_genre CHAR(20))
BEGIN
